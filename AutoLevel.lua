...
-- [[ THỜI GIAN BẮT ĐẦU TREO ]]
local StartTime = tick()

-- [[ CỜ CHỜ HỒI SINH CHO RESET TELE ]]
_G.WaitingForRespawn = false
...
-- [[ TỰ ĐỘNG PHÂN BIỆT THẾ GIỚI ]]
local PlaceId = game.PlaceId
local World1 = (PlaceId == 2753915549)
local World2 = (PlaceId == 4442272125)
...
-- [[ HÀM DI CHUYỂN BẰNG CÁCH THAY ĐỔI TRẠNG THÁI ]]
...
-- [[ RESET TELE - SỬA LỖI ]]
local function ResetTele(targetCFrame)
    if not targetCFrame then return end
    local char = LocalPlayer.Character
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
    _G._resetTarget = targetCFrame
    _G.WaitingForRespawn = true
    hum.Health = 0
end

-- SỰ KIỆN HỒI SINH - SỬA LỖI
LocalPlayer.CharacterAdded:Connect(function(newChar)
    if _G._resetTarget then
        task.wait(0.5)
        local root = newChar:FindFirstChild("HumanoidRootPart")
        if root then
            root.CFrame = _G._resetTarget
            root.AssemblyLinearVelocity = Vector3.zero
            root.AssemblyAngularVelocity = Vector3.zero
...
local function EquipWeapon(weaponName)
    ...
    if Config.AutoMelee then
        ...
        if not currentTool then
            ...
            if firstTool then LocalPlayer.Character.Humanoid:EquipTool(firstTool) end
        end
    end
end

local function AutoHaki() end

-- [[ HÀM LƯU TRÁI ]]
local function UpdStFruit()
    pcall(function()
        local rep = game:GetService("ReplicatedStorage")
        rep.Remotes.CommF_:InvokeServer("StoreFruits")
    end)
end
...
task.spawn(function()
    local RedeemCodes = {
        ...
        "StrawHatMaine", "Sub2OfficialNoobie", "Fudd10", "Bignews",
        "TheGreatAce", "SECRET_ADMIN", "SUB2GAMERROBOT_RESET1",
    ...
    Notify("Hệ Thống", "Đã nhập xong " .. #RedeemCodes .. " code!")
end)

-- [[ LOGIC LẤY NHIỆM VỤ - CHECKQUEST ]]
function CheckQuest()
    pcall(function()
        local MyLevel = LocalPlayer.Data.Level.Value
        Mon = nil; LevelQuest = nil; NameQuest = nil; NameMon = nil; CFrameQuest = nil; CFrameMon = nil
        ...
        if World1 then
            ...
            elseif (MyLevel >= 375 and MyLevel <= 399) or SelectMonster == "Fishman Warrior" then
                ...
                if _G.AutoFarm and (CFrameQuest.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 10000 then
        ...
        elseif World2 then
            ...
            elseif (MyLevel >= 1175 and MyLevel <= 1199) or SelectMonster == "Magma Ninja" then
                ...
                CFrameMon = CFrame.new(-5449.6728515625, 76.65874481101172, -5808.20068359375)
        ...
        elseif World3 then
            ...
            elseif (MyLevel >= 1575 and MyLevel <= 1599) or SelectMonster == "Dragon Crew Warrior" then
                ...
                CFrameMon = CFrame.new(6709.76367, 52.3442993, -1139.02966, -0.763515472, 0, 0.645789504, 0, 1, 0, -0.645789504, -0.763515472)
...
task.spawn(function()
    while task.wait(0.1) do
        if Config.AutoFarm and Config.BringMob then
            pcall(function()
                CheckQuest()
                ...
                local function processMob(mob)
                    if mob:IsA("Model") and mob.Name == Mon and mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 and mob:FindFirstChild("HumanoidRootPart") then
                        local hrp = mob.HumanoidRootPart
                        local dist = (hrp.Position - playerPos).Magnitude
                        if dist <= 50 then
                            hrp.CFrame = rootPart.CFrame * CFrame.new(0, 0, -5)
                            hrp.CanCollide = false
                            if not hrp:FindFirstChild("HoldVelocity") then
                ...
                for _, mob in pairs(workspace:GetChildren()) do
                    if not (enemies and mob.Parent == enemies) then
                        processMob(mob)
                    end
                end
            end)
...
task.spawn(function()
    while task.wait(0.5) do
        if Config.AutoStats then
            pcall(function()
                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AddPoint", Config.StatTarget, 1)
            end)
        end
    end
end)

-- [[ AUTO FARM CHÍNH - SỬA LỖI RESET TELE ]]
spawn(function()
    while task.wait() do
        if _G.AutoFarm then
            pcall(function()
                -- Nếu đang chờ hồi sinh, bỏ qua
                if _G.WaitingForRespawn then return end
                ...
                local questTitle = questContainer:FindFirstChild("Container")
                if not questTitle then return end
                local titleLabel = questTitle:FindFirstChild("QuestTitle")
                if not titleLabel then return end
                local titleText = titleLabel:FindFirstChild("Title")
                if not titleText then return end
                local v673 = titleText.Text
                ...
                if questContainer.Visible == false then
                    StartBring = false
                    _G.CurrentTarget = nil
                    if Config.ResetTele and CFrameQuest and (LocalPlayer.Character.HumanoidRootPart.Position - CFrameQuest.Position).Magnitude > 500 then
                        if not _G.WaitingForRespawn then
                            ResetTele(CFrameQuest)
                        end
                    else
                        if not _G.WaitingForRespawn then
                            TP1(CFrameQuest)
                    ...
                    if Config.ResetTele and CFrameMon and (LocalPlayer.Character.HumanoidRootPart.Position - CFrameMon.Position).Magnitude > 500 then
                        if not _G.WaitingForRespawn then
                            ResetTele(CFrameMon)
                        end
                    else
                    ...
                    if string.find(v673, "kissed") then
                        local enemies = workspace:FindFirstChild("Enemies")
                        if enemies then
                            for _, v677 in pairs(enemies:GetChildren()) do
                                if string.find(v677.Name, "kissed Warrior") and v677:FindFirstChild("HumanoidRootPart") and v677:FindFirstChild("Humanoid") and v677.Humanoid.Health > 0 then
                                    if string.find(v673, NameMon) then
                                        _G.CurrentTarget = v677
                                        repeat
                                            task.wait()
                                            EquipWeapon(_G.SelectWeapon)
                                            PosMon = v677.HumanoidRootPart.CFrame
                                            if not Config.LockCFrame then
                                                TP1(v677.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                            end
                                        ...
                                        until not _G.AutoFarm or v677.Humanoid.Health <= 0 or not v677.Parent or questContainer.Visible == false or _G.WaitingForRespawn
                                        _G.CurrentTarget = nil
                                    else
                                        StartBring = false
                                        _G.CurrentTarget = nil
                        ...
                        if targetMonster then
                            if string.find(v673, NameMon) then
                                _G.CurrentTarget = targetMonster
                                repeat
                                    task.wait()
                                    EquipWeapon(_G.SelectWeapon)
                                    AutoHaki()
                                    PosMon = targetMonster.HumanoidRootPart.CFrame
                                    if not Config.LockCFrame then
                                        TP1(targetMonster.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    end
                                    targetMonster.HumanoidRootPart.CanCollide = false
                                    targetMonster.Humanoid.WalkSpeed = 0
                                    targetMonster.Head.CanCollide = false
                                    targetMonster.HumanoidRootPart.Size = Vector3.new(70, 70, 70)
                                    StartBring = true
                                    MonFarm = targetMonster.Name
                                    VirtualUser:CaptureController()
                                    VirtualUser:Button1Down(Vector2.new(1280, 672))
                                until not _G.AutoFarm or targetMonster.Humanoid.Health <= 0 or not targetMonster.Parent or questContainer.Visible == false or _G.WaitingForRespawn
                                _G.CurrentTarget = nil
                            else
                                StartBring = false
                                _G.CurrentTarget = nil
                            ...
                            StartBring = false
                            _G.CurrentTarget = nil
                            local repMon = game:GetService("ReplicatedStorage"):FindFirstChild(Mon)
                            if repMon and repMon:FindFirstChild("HumanoidRootPart") then
                                TP1(repMon.HumanoidRootPart.CFrame * CFrame.new(0, 20, 0))
                            end
                        end
                    end
                end
            ...
            if getgenv()._G.KillAura then
                ...
                if rootPart and enemiesFolder then
                    for _, enemy in pairs(enemiesFolder:GetChildren()) do
                        ...
                        if enemyRoot and (rootPart.Position - enemyRoot.Position).Magnitude <= 65 then
                            FireAttackRemote(enemy)
                            break
                        end
                    end
                end
...
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            ...
            local questContainer = questGui:FindFirstChild("Quest")
            if not questContainer then return end
            if questContainer.Visible == false then return end
            ...
            if dist > 500 then
                ResetTele(targetCF)
                -- Chờ hồi sinh tối đa 5 giây
                local timeout = 0
                while _G.WaitingForRespawn and timeout < 5 do
                    task.wait(0.5)
                    timeout = timeout + 0.5
                end
            end
...
-- =========================================================
-- ============ KHOÁ CFRAME (CFRAME LOCKING) ===============
-- =========================================================
task.spawn(function()
    RunService.Heartbeat:Connect(function()
        if Config.LockCFrame and _G.AutoFarm and _G.CurrentTarget and _G.CurrentTarget.Parent and _G.CurrentTarget:FindFirstChild("Humanoid") and _G.CurrentTarget.Humanoid.Health > 0 then
            pcall(function()
                local char = LocalPlayer.Character
                local root = char and char:FindFirstChild("HumanoidRootPart")
                local targetRoot = _G.CurrentTarget:FindFirstChild("HumanoidRootPart")
                if root and targetRoot then
                    ...
                    root.CFrame = targetRoot.CFrame * CFrame.new(0, Config.LockOffset, 0)
                    root.AssemblyLinearVelocity = Vector3.zero
                    root.AssemblyAngularVelocity = Vector3.zero
...
task.spawn(function()
    while task.wait(0.2) do
        pcall(function()
            local rep = game:GetService("ReplicatedStorage")
            if _G.AutoSaber and LocalPlayer.Data.Level.Value >= 200
               and not LocalPlayer.Backpack:FindFirstChild("Saber")
               and not LocalPlayer.Character:FindFirstChild("Saber") then
                -- (giữ nguyên logic cũ, nhưng không tắt auto farm)
                -- Tuy nhiên để tránh xung đột, tạm thời không làm gì thêm
                if workspace.Map.Jungle.Final.Part.Transparency == 0 then
                    if workspace.Map.Jungle.QuestPlates.Door.Transparency == 0 then
                        if (CFrame.new(-1612.55884, 36.9774132, 148.719543, 0.37091279, 3.0717151e-09, -0.928667724, 3.97099491e-08, 1, 1.91679348e-08, 0.928667724, -4.39869794e-08, 0.37091279).Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 100 then
                        ...
                        if workspace.Map.Desert.Burn.Part.Transparency == 0 then
                            ...
                            if rep.Remotes.CommF_:InvokeServer("ProQuestProgress", "SickMan") ~= 0 then
                                rep.Remotes.CommF_:InvokeServer("ProQuestProgress", "GetCup")
                                task.wait(0.5)
                                EquipWeapon("Cup")
                                task.wait(0.5)
                                rep.Remotes.CommF_:InvokeServer("ProQuestProgress", "FillCup", LocalPlayer.Character.Cup)
                                task.wait(3)
                                rep.Remotes.CommF_:InvokeServer("ProQuestProgress", "SickMan")
                            else
                                local richSonStatus = rep.Remotes.CommF_:InvokeServer("ProQuestProgress", "RichSon")
                                if richSonStatus == nil then
                                    rep.Remotes.CommF_:InvokeServer("ProQuestProgress", "RichSon")
                                elseif richSonStatus == 0 then
                                    if workspace.Enemies:FindFirstChild("Mob Leader") or rep:FindFirstChild("Mob Leader") then
                                        ...
                                        for _, v in pairs(workspace.Enemies:GetChildren()) do
                                            if v.Name == "Mob Leader" and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and v.Parent then
                                                repeat
                                                    task.wait()
                                                    EquipWeapon(_G.SelectWeapon)
                                                    ...
                                                    VirtualUser:CaptureController()
                                                    VirtualUser:Button1Down(Vector2.new(1280, 672))
                                                until v.Humanoid.Health <= 0 or _G.AutoSaber == false or _G.WaitingForRespawn
                                            end
                                        end
                                    end
                    ...
                    if workspace.Enemies:FindFirstChild("Saber Expert") or rep:FindFirstChild("Saber Expert") then
                        for _, v in pairs(workspace.Enemies:GetChildren()) do
                            if v.Name == "Saber Expert" and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and v.Parent then
                                repeat
                                    task.wait()
                                    EquipWeapon(_G.SelectWeapon)
                                    TP1(v.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                ...
                                if v.Humanoid.Health <= 0 then
                                    rep.Remotes.CommF_:InvokeServer("ProQuestProgress", "PlaceRelic")
                                end
                            end
                        end
                    else
        ...
        if _G.AutoSecondSea then
            pcall(function()
                if LocalPlayer.Data.Level.Value >= 700 and World1 then
                    ...
                    if iceDoor.CanCollide ~= true or iceDoor.Transparency ~= 0 then
                        if iceDoor.CanCollide ~= false or iceDoor.Transparency ~= 1 then
                            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelDressrosa")
                        elseif workspace.Enemies:FindFirstChild("Ice Admiral") then
                            for _, v in pairs(workspace.Enemies:GetChildren()) do
                                if v.Name == "Ice Admiral" and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
                                    repeat
                                        ...
                                        VirtualUser:CaptureController()
                                        VirtualUser:Button1Down(Vector2.new(1280, 672))
                                    until v.Humanoid.Health <= 0 or not (v.Parent and _G.AutoSecondSea) or _G.WaitingForRespawn
                                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelDressrosa")
                                end
                            end
                        else
                        ...
                        local tpTarget = CFrame.new(4851.8720703125, 5.6514348983765, 718.47094726563)
                        repeat
                            task.wait()
                            topos(tpTarget)
                        until (tpTarget.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.AutoSecondSea or _G.WaitingForRespawn
                        task.wait(1)
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("DressrosaQuestProgress", "Detective")
                        EquipWeapon("Key")
                        local keyTarget = CFrame.new(1347.7124, 37.3751602, -1325.6488)
                        repeat
                            task.wait()
                            topos(keyTarget)
                        until (keyTarget.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.AutoSecondSea or _G.WaitingForRespawn
                        task.wait(3)
                    end
                    -- Khôi phục trạng thái farm
                    _G.AutoFarm = wasFarming
                    _G.AutoSecondSea = false -- tắt để không lặp lại
                end
            end)
        end
    end
        ...
        if _G.ThirdSea then
            pcall(function()
                if LocalPlayer.Data.Level.Value >= 1500 and World2 then
                    ...
                    _G.AutoFarm = false
                    local rep = game:GetService("ReplicatedStorage")
                    if rep.Remotes.CommF_:InvokeServer("ZQuestProgress", "General") == 0 then
                        local beginCF = CFrame.new(-1926.3221435547, 12.819851875305, 1738.3092041016)
                        topos(beginCF)
                        if (beginCF.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 10 then
                            task.wait(1.5)
                            rep.Remotes.CommF_:InvokeServer("ZQuestProgress", "Begin")
                        end
                        task.wait(1.8)
                            ...
                            for _, v in pairs(workspace.Enemies:GetChildren()) do
                                if v.Name == "rip_indra" then
                                    ...
                                    repeat
                                        ...
                                        rep.Remotes.CommF_:InvokeServer("TravelZou")
                                        sethiddenproperty(LocalPlayer, "SimulationRadius", math.huge)
                                    until _G.ThirdSea == false or v.Humanoid.Health <= 0 or not v.Parent or _G.WaitingForRespawn
                                end
                            end
                        elseif (CFrame.new(-26880.93359375, 22.848554611206, 473.18951416016).Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 1000 then
...
ScreenGui.Name = "MinhChien"; ScreenGui.ResetOnSpawn = false
...
local function Drag(gui)
    ...
    gui.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then drag = false end end)
end

-- BẢNG THÔNG TIN
local InfoBoard = Instance.new("Frame", ScreenGui)
InfoBoard.Size = UDim2.new(0, 220, 0, 75); InfoBoard.Position = UDim2.new(0, 10, 0, 10)
InfoBoard.BackgroundColor3 = Color3.fromRGB(15, 15, 25); InfoBoard.Active = true; Drag(InfoBoard)
Instance.new("UICorner", InfoBoard).CornerRadius = UDim.new(0, 8)
local InfoStroke = Instance.new("UIStroke", InfoBoard); InfoStroke.Thickness = 2; InfoStroke.Color = Color3.fromRGB(75, 255, 75)

local InfoText = Instance.new("TextLabel", InfoBoard)
InfoText.Size = UDim2.new(1, 0, 1, 0); InfoText.BackgroundTransparency = 1
InfoText.TextColor3 = Color3.new(1, 1, 1); InfoText.Font = Enum.Font.GothamBold; InfoText.TextSize = 12
InfoText.Text = "Đang tải dữ liệu..."
...
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            ...
            InfoText.Text = string.format("Level: %d\nMoney: $%s\nThời gian treo: %s", level, formattedBeli, formatTime(elapsed))
        end)
    end
end)
...
local Main = Instance.new("Frame", ScreenGui); Main.Size = UDim2.new(0, 240, 0, 600); Main.Position = UDim2.new(0.5, -120, 0.15, 0); Main.BackgroundColor3 = Color3.fromRGB(5, 5, 10); Main.Visible = false; Main.Active = true; Drag(Main)
...
local Title = Instance.new("TextLabel", Main); Title.Size = UDim2.new(1, 0, 0, 40); Title.Text = "MINHCHIEN"; Title.TextColor3 = Color3.new(1, 1, 1); Title.Font = Enum.Font.GothamBlack; Title.TextSize = 14; Title.BackgroundTransparency = 1
local Holder = Instance.new("ScrollingFrame", Main); Holder.Size = UDim2.new(0.9, 0, 0.85, 0); Holder.Position = UDim2.new(0.05, 0, 0.1, 0); Holder.BackgroundTransparency = 1; Holder.CanvasSize = UDim2.new(0, 0, 2.5, 0); Holder.ScrollBarThickness = 0
...
local function CreateButton(text, startState, callback)
    local Btn = Instance.new("TextButton", Holder)
    Btn.Size = UDim2.new(0.95, 0, 0, 42); Btn.BackgroundColor3 = Color3.fromRGB(15, 15, 25)
    ...
    local function Update()
        if state then
            Btn.Text = text .. ": ON"; Btn.TextColor3 = Color3.fromRGB(75, 255, 75); Stroke.Color = Color3.fromRGB(75, 255, 75)
        else
            Btn.Text = text .. ": OFF"; Btn.TextColor3 = Color3.fromRGB(255, 75, 75); Stroke.Color = Color3.fromRGB(40, 40, 60)
        end
    end
    Update()
    ...
    Btn.Activated:Connect(function()
        state = not state
        Update()
        Notify("Hệ Thống", text .. " " .. (state and "Đã Bật" or "Đã Tắt"))
        callback(state)
    end)
    return Btn
end

CreateButton("Auto Farm", Config.AutoFarm, function(s) Config.AutoFarm = s; _G.AutoFarm = s end)
CreateButton("Kill Aura", Config.KillAura, function(s) Config.KillAura = s; getgenv()._G.KillAura = s end)
CreateButton("Gom Quái", Config.BringMob, function(s) Config.BringMob = s end)
CreateButton("Auto Melee", Config.AutoMelee, function(s) Config.AutoMelee = s end)
CreateButton("Auto Stats", Config.AutoStats, function(s) Config.AutoStats = s end)
CreateButton("Bypass TP", BypassTP, function(s) BypassTP = s end)
CreateButton("Reset Tele", Config.ResetTele, function(s) Config.ResetTele = s end)
CreateButton("Khóa CFrame", Config.LockCFrame, function(s) Config.LockCFrame = s end)
CreateButton("Auto Saber", _G.AutoSaber, function(s) _G.AutoSaber = s end)
CreateButton("Auto 2nd Sea", _G.AutoSecondSea, function(s) _G.AutoSecondSea = s end)
CreateButton("Auto 3rd Sea", _G.ThirdSea, function(s) _G.ThirdSea = s end)

local StatOptions = {"Melee", "Defense", "Sword", "Gun", "Demon Fruit"}
local StatIndex = 1

local SelectStatBtn = Instance.new("TextButton", Holder)
SelectStatBtn.Size = UDim2.new(0.95, 0, 0, 42); SelectStatBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 45)
SelectStatBtn.Text = "Cộng vào: " .. Config.StatTarget
SelectStatBtn.TextColor3 = Color3.fromRGB(255, 200, 50); SelectStatBtn.Font = Enum.Font.GothamBold; SelectStatBtn.TextSize = 12
Instance.new("UICorner", SelectStatBtn).CornerRadius = UDim.new(0, 8)
Instance.new("UIStroke", SelectStatBtn).Color = Color3.fromRGB(60, 60, 80)

SelectStatBtn.Activated:Connect(function()
    StatIndex = StatIndex + 1
    if StatIndex > #StatOptions then StatIndex = 1 end
    Config.StatTarget = StatOptions[StatIndex]
    SelectStatBtn.Text = "Cộng vào: " .. Config.StatTarget
end)nd)d)