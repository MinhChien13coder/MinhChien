...
-- [[ HÀM DI CHUYỂN THÔNG MINH (TÍCH HỢP RESET TELE) ]]
local function topos(targetCFrame)
    -- Nếu ResetTele bật, chưa đang hồi sinh, và khoảng cách > 500
    if Config.ResetTele and not _G.WaitingForRespawn then
        local char = LocalPlayer.Character
        if char then
            ...
            if root and targetCFrame then
                ...
                if dist > 500 then
                    -- Kiểm tra xem có cầm tool không (vũ khí, key, trái, …)
                    local tool = char:FindFirstChildOfClass("Tool")
                    if not tool then
                        -- Không cầm gì -> dùng Reset Tele
                        ResetTele(targetCFrame)
                        -- Chờ hồi sinh tối đa 5s
                        local timeout = 0
                        while _G.WaitingForRespawn and timeout < 5 do
                            task.wait(0.5)
                            timeout = timeout + 0.5
                        end
                        return -- thoát, không thực hiện bay
                    end
                end
            end
        end
    ...
    -- Nếu không reset, thực hiện bay thông thường
    pcall(function()
        local char = LocalPlayer.Character
        ...
        if _G.AutoFarm then
            pcall(function()
                ...
                if questContainer.Visible == false then
                    StartBring = false
                    _G.CurrentTarget = nil
                    -- Sử dụng TP1 (đã tích hợp reset tele)
                    TP1(CFrameQuest)
