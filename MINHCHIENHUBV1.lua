...
local LangConfig = {
    VI = {
        ...
        openDexBtn = "MỞ DARK DEX V3 📁",
        openSpyBtn = "MỞ REMOTE SPY 🕵️‍♂️"
    },
    EN = {
        mainTab = "MAIN",
        toolTab = "TOOLS",
...
ToolTabFrame.CanvasSize = UDim2.new(0, 0, 2.0, 0)
...
SettingsTabFrame.CanvasSize = UDim2.new(0, 0, 2.8, 0)
...
-- 6. FLY (ĐÃ TÍCH HỢP BYPASS ANTI-FALL / RUBBERBAND)
local flyBtn = CreateButton("fly", MainTabFrame)
local flyToggle = false
local bv, bg = nil, nil

-- Tạo sẵn bệ đỡ tàng hình cho Anti-Cheat Bypass
local flyPlatform = Instance.new("Part")
flyPlatform.Name = "AntiFlyBypass"
...
flyPlatform.CFrame = CFrame.new(0, 999999, 0) -- Giấu đi khi chưa bật fly
        ...
        if root and hum then
            ...
            task.spawn(function()
                ...
                while flyToggle and char and root and hum.Parent do
                    ...
                    -- Tính toán tốc độ bay dựa trên hướng nhìn camera
                    if hum.MoveDirection.Magnitude > 0 then 
                        bv.velocity = camera.CFrame.LookVector * flySpeed 
                    else 
                        bv.velocity = Vector3.new(0, 0.15 * math.sin(tick() * 10), 0) 
                    end
                    
                    -- KÍCH HOẠT BYPASS ANTI-FALL (PLATFORM SPOOFING)
                    -- Liên tục lót gạch tàng hình ngay dưới chân để đánh lừa máy chủ
                    flyPlatform.CFrame = root.CFrame * CFrame.new(0, -3.2, 0)
                    
                    -- Buộc ép vận tốc rơi (Y) về 0 trên Server
                    root.Velocity = Vector3.new(root.Velocity.X, 0, root.Velocity.Z)
                    
                    -- Thay đổi trạng thái nhân vật thành đang chạy thay vì đang rơi tự do
                ...
                flyPlatform.CFrame = CFrame.new(0, 999999, 0) -- Cất bệ đỡ
            end)
        end
    else
        if bg then bg:Destroy() end 
...
-- ================= TAB SETTINGS: CẤU HÌNH NGÔN NGỮ & SLIDER =================

-- NÚT THÊM MỚI 1: OPEN DARK DEX V3
local dexBtn = CreateButton("openDexBtn", SettingsTabFrame)
dexBtn.BackgroundColor3 = Color3.fromRGB(25, 30, 45)

RegisterUIUpdate(function()
    dexBtn.Text = LangConfig[currentLanguage].openDexBtn
end)

dexBtn.MouseButton1Click:Connect(function()
    ClickSound:Play()
    pcall(function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/infyyds/FTM/main/DexV3"))()
    end)
end)

-- NÚT THÊM MỚI 2: OPEN REMOTE SPY
local spyBtn = CreateButton("openSpyBtn", SettingsTabFrame)
spyBtn.BackgroundColor3 = Color3.fromRGB(35, 25, 40)

RegisterUIUpdate(function()
    spyBtn.Text = LangConfig[currentLanguage].openSpyBtn
end)

spyBtn.MouseButton1Click:Connect(function()
    ClickSound:Play()
    pcall(function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/7GrandDadPGN/VapeV4ForRoblox/main/CustomModules/RemoteSpy.lua"))()
    end)
end)
...
RegisterUIUpdate(function()
    changeLangBtn.Text = LangConfig[currentLanguage].changeLangBtn
end)
...
RegisterUIUpdate(function()
    hopBtn.Text = LangConfig[currentLanguage].serverHop
end)
...
hopBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        local success, err = pcall(function()
            ...
            local teleportUrl = "https://games.roblox.com/v1/places/" .. placeId .. "/servers/Public?limit=100"
            local cursor = ""
            local targetServer = nil

            for i = 1, 3 do
                local reqUrl = teleportUrl .. (cursor ~= "" and "&cursor=" .. cursor or "")
                local rawData = game:HttpGet(reqUrl)
                local serverData = HttpService:JSONDecode(rawData)

                if serverData and serverData.data then
                    for _, server in pairs(serverData.data) do
                        if type(server) == "table" and server.id ~= jobId and tonumber(server.playing) and tonumber(server.maxPlayers) then
            ...
            if targetServer then
                ...
                hopBtn.Text = (currentLanguage == "VI" and "❌ KHÔNG TÌM THẤY! THỬ LẠI..." or "❌ FAILED! RETRYING...")
                hopBtn.TextColor3 = Color3.fromRGB(255, 100, 100)
                task.wait(1.5)
...
RegisterUIUpdate(function()
    rejoinBtn.Text = LangConfig[currentLanguage].rejoinServer
end)
