...
local function CheckAndGetQuest()
    ...
    local hasQuest = playerGui and playerGui:FindFirstChild("Main") and playerGui.Main:FindFirstChild("Quest") and playerGui.Main.Quest.Visible
    
    if not hasQuest and CFrameQuest then
        topos(CFrameQuest)
        task.wait(0.2)
...
local function EquipTargetWeapon()
    ...
    if Config.AutoMelee then
        ...
        if not currentTool then
            ...
            if firstTool then character.Humanoid:EquipTool(firstTool) end
        end
    end
end

-- [[ LOGIC FARM VÀ ĐÁNH QUÁI ]]
local function FarmTargetMonster()
    local character = LocalPlayer.Character
    ...
    if targetMob then
        while Config.AutoFarm and targetMob.Parent and targetMob.Humanoid.Health > 0 and character:FindFirstChild("Humanoid") and character.Humanoid.Health > 0 do
            task.wait()
            local targetPos = targetMob.HumanoidRootPart.CFrame * CFrame.new(0, Config.Distance, 0) * CFrame.Angles(math.rad(-90), 0, 0)
            topos(targetPos)
            EquipTargetWeapon()
        ...
        if CFrameMon then topos(CFrameMon) end
    end
end
...
task.spawn(function()
    while task.wait(0.1) do
        if Config.AutoFarm then
            pcall(function() CheckQuest(); CheckAndGetQuest(); FarmTargetMonster() end)
        end
    end
end)
    ...
    while task.wait() do
        if getgenv()._G.KillAura then
            pcall(function()
                ...
                if rootPart and char:FindFirstChildOfClass("Tool") then
                    for _, mob in pairs(workspace:GetChildren()) do
                        if mob:IsA("Model") and mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 and mob:FindFirstChild("HumanoidRootPart") then
                            if not Players:GetPlayerFromCharacter(mob) and (rootPart.Position - mob.HumanoidRootPart.Position).Magnitude <= 65 then
                                FireAttackRemote(mob)
                            end
                        end
                    end
                end
...
-- Vòng lặp cập nhật Bảng Thông Tin
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            ...
            -- Format số tiền thêm dấu phẩy (vd: 1,000,000)
...
local Main = Instance.new("Frame", ScreenGui); Main.Size = UDim2.new(0, 240, 0, 480); Main.Position = UDim2.new(0.5, -120, 0.2, 0); Main.BackgroundColor3 = Color3.fromRGB(5, 5, 10); Main.Visible = false; Main.Active = true; Drag(Main)
...
local Holder = Instance.new("ScrollingFrame", Main); Holder.Size = UDim2.new(0.9, 0, 0.85, 0); Holder.Position = UDim2.new(0.05, 0, 0.1, 0); Holder.BackgroundTransparency = 1; Holder.CanvasSize = UDim2.new(0, 0, 1.8, 0); Holder.ScrollBarThickness = 0
