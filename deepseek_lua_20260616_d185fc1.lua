...
_G.CurrentTarget = nil      -- Quái đang đánh (dùng cho Lock CFrame)
...
-- [[ HÀM DI CHUYỂN BẰNG CÁCH THAY ĐỔI TRẠNG THÁI (ANTI-RUBBERBAND) ]]
...
-- [[ TỰ ĐỘNG NHẬP CODE KHI BẬT SCRIPT ]]
task.spawn(function()
    local RedeemCodes = {
                ...
                local function processMob(mob)
                    if mob:IsA("Model") and mob.Name == Mon and mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 and mob:FindFirstChild("HumanoidRootPart") then
                        ...
                        if dist <= 35 then
                            ...
                            if not mob.HumanoidRootPart:FindFirstChild("HoldVelocity") then
                                local bv = Instance.new("BodyVelocity", mob.HumanoidRootPart)
...
-- [[ AUTO FARM NÂNG CAO + RESET TELE + GÁN QUÁI MỤC TIÊU CHO LOCK ]]
spawn(function()
    while task.wait() do
        if _G.AutoFarm then
            pcall(function()
                ...
                if questContainer.Visible == false then
                    ...
                    if string.find(v673, "kissed") then
                            ...
                            for _, v677 in pairs(enemies:GetChildren()) do
                                if string.find(v677.Name, "kissed Warrior") and v677:FindFirstChild("HumanoidRootPart") and v677:FindFirstChild("Humanoid") and v677.Humanoid.Health > 0 then
                                    if string.find(v673, NameMon) then
                                        _G.CurrentTarget = v677  -- Gán quái mục tiêu
                                        repeat
                                            task.wait()
                                        ...
                                        until not _G.AutoFarm or v677.Humanoid.Health <= 0 or not v677.Parent or questContainer.Visible == false
                                        _G.CurrentTarget = nil
                                    else
                                        StartBring = false
                                        _G.CurrentTarget = nil
                        ...
                        if targetMonster then
                            if string.find(v673, NameMon) then
                                _G.CurrentTarget = targetMonster  -- Gán quái mục tiêu
                                repeat
                                    task.wait()
                                    EquipWeapon(_G.SelectWeapon)
                                    AutoHaki()
                                    ...
                                    VirtualUser:CaptureController()
                                    VirtualUser:Button1Down(Vector2.new(1280, 672))
                                until not _G.AutoFarm or targetMonster.Humanoid.Health <= 0 or not targetMonster.Parent or questContainer.Visible == false
                                _G.CurrentTarget = nil
                            else
                                StartBring = false
                                _G.CurrentTarget = nil
                ...
                if LocalPlayer.Data.Level.Value >= 700 and World1 then
                    ...
                    if iceDoor.CanCollide ~= true or iceDoor.Transparency ~= 0 then
                            ...
                            for _, v in pairs(workspace.Enemies:GetChildren()) do
                                if v.Name == "Ice Admiral" and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
                                    repeat
                                        ...
                                        VirtualUser:CaptureController()
                                        VirtualUser:Button1Down(Vector2.new(1280, 672))
                                    until v.Humanoid.Health <= 0 or not (v.Parent and _G.AutoSecondSea)
                                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelDressrosa")
                                end
                            end
                        else
                        ...
                        repeat
                            task.wait()
                            topos(tpTarget)
                        until (tpTarget.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.AutoSecondSea
                        task.wait(1)
                        ...
                        repeat
                            task.wait()
                            topos(keyTarget)
                        until (keyTarget.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.AutoSecondSea
                        task.wait(3)
                    end
                end
            end)
